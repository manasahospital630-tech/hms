...
const createUser = async (input) => {
    const existing = await (0, database_1.query)('SELECT user_id FROM users WHERE email = $1', [input.email]);
...
const upsertDoctorProfile = async (input) => {
    const user = await (0, database_1.query)("SELECT role FROM users WHERE user_id = $1", [input.doctorId]);
...
const getHospitalSettings = async () => {
    const result = await (0, database_1.query)('SELECT * FROM hospital_settings WHERE id = 1');
    return result.rows[0];
};
exports.getHospitalSettings = getHospitalSettings;
const updateHospitalSettings = async (input) => {
    let logoUrl = input.hospitalLogo || null;
    if (input.hospitalLogo && input.hospitalLogo.startsWith('data:image/')) {
        try {
            logoUrl = await (0, s3Upload_1.uploadBase64Image)(input.hospitalLogo, 'logos');
    ...
    const result = await (0, database_1.query)(`UPDATE hospital_settings
     SET hospital_name = $1,
         hospital_address = $2,
         phone_number = $3,
         website = $4,
...
const getDashboardStats = async () => {
    const [docTotalRes, presentDocsRes, nurseTotalRes, otherStaffRes, opBookedRes, revOverallRes, billsTotalRes, revTodayRes, ipBillsRes, bedsTotalRes, bedsAvailRes, bedsOccRes, activityRes] = await Promise.all([
        (0, database_1.query)("SELECT COUNT(*) as count FROM users WHERE role = 'Doctor' AND is_active = TRUE"),
        (0, database_1.query)("SELECT COUNT(DISTINCT doctor_id) as count FROM appointments WHERE DATE(appointment_date) = CURRENT_DATE"),
        (0, database_1.query)("SELECT COUNT(*) as count FROM users WHERE role = 'Nurse' AND is_active = TRUE"),
        (0, database_1.query)("SELECT COUNT(*) as count FROM users WHERE role IN ('Receptionist', 'Pharmacist', 'Biller', 'Management') AND is_active = TRUE"),
        (0, database_1.query)("SELECT COUNT(*) as count FROM appointments WHERE DATE(appointment_date) = CURRENT_DATE"),
        (0, database_1.query)("SELECT COALESCE(SUM(total_amount), 0) as count FROM invoices WHERE status = 'Paid'"),
        (0, database_1.query)("SELECT COUNT(*) as count FROM invoices"),
        (0, database_1.query)("SELECT COALESCE(SUM(total_amount), 0) as count FROM invoices WHERE DATE(created_at) = CURRENT_DATE AND status = 'Paid'"),
        (0, database_1.query)("SELECT COUNT(*) as count FROM invoices WHERE ip_admission_id IS NOT NULL"),
        (0, database_1.query)("SELECT COUNT(*) as count FROM hospital_beds"),
        (0, database_1.query)("SELECT COUNT(*) as count FROM hospital_beds WHERE status = 'Available'"),
        (0, database_1.query)("SELECT COUNT(*) as count FROM hospital_beds WHERE status = 'Occupied'"),
        (0, database_1.query)(`
...
const getConsolidatedHospitalRevenue = async (options) => {
    const selectedTf = options.selectedTimeframe || options.period || 'today';
    // Helper query generator for date filter
    const buildQueriesForPeriod = async (periodKey, customStart, customEnd) => {
        ...
        const billingRes = await (0, database_1.query)(`
      SELECT 
        COUNT(*) as count,
